...
export default async function transformGroupedDataForApex(groupedData: GroupedData[] | undefined): Promise<RawSensorData[]> {
  const sensorMap = new Map<string, { sensorName: string; sensorLocation: string; labels: string[]; data: number[] }>();
  ...
  if (groupedData) {
    for (const entry of groupedData) {
      ...
      const limitValue = additional_sensor_info[0]?.limitValue ?? 6000;
      ...
      if (!sensorMap.has(entry.sensor_id)) {
        sensorMap.set(entry.sensor_id, { sensorName, sensorLocation, labels: [], data: [] });
      }
      ...
      if (sensorData) {
        ...
        if (typeof transformedCode === "number") {
          sensorData.data.push(transformedCode);
        }
      }
    }
  }
  ...
  sensorMap.forEach(({ sensorName, sensorLocation, labels, data }) => {
