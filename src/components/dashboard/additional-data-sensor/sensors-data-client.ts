...
export class SensorsDataClient {
  ...
  async getLastValuesDataForDynamicCharts(objectId: string, sensorIds: NewSensor[] ): Promise<ApiResult> {
    try {
      ...
      const response = await fetch(`${BASE_URL}/sensors_data/get_last_values_data_for_dynamic_charts`, {
        method: 'POST',
        headers: {
          ...headers,
